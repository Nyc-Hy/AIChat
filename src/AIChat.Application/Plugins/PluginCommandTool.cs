...
public sealed class PluginCommandTool : IAgentTool
{
    ...
    private PluginCommand BuildCommand(string argumentsJson, AgentToolContext context)
    {
        ...
        var workingDirectory = Expand(_tool.Command.WorkingDirectory, variables);
        ...
        {
            workingDirectory = ProjectPathGuard.ResolveInsideProject(context.ProjectPath, workingDirectory);
        }

        var executable = Expand(_tool.Command.Executable, variables);
    ...
    private static string Expand(string template, IReadOnlyDictionary<string, string> variables)
    {
