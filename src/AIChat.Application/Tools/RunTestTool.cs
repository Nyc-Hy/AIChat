...
public sealed class RunTestTool : IAgentTool
{
    ...
    {
        Name = "run_test",
        Description = "在当前项目中执行 dotnet test。用于运行测试并返回结构化输出，只允许测试项目内目标。",
        ParametersJson = """
        {
          "type": "object",
          "properties": {
            ...
            "timeout_seconds": { "type": "integer", "description": "超时时间，默认 180，最大 600。" },
    ...
    {
        return DotnetVerification.RunAsync(Id, context.ProjectPath, "test", argumentsJson, cancellationToken);
    }
}
